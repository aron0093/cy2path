...
def compute_conditionals(adata, use_selected=True):
    ...
    observed_state_probs_mean = (
        joint_probs.mean(0) / joint_probs.mean(0)[selected_states].sum()
    )
    ...
    ] = observed_state_probs_mean.sum(1) / observed_state_probs_mean.sum(1).sum(0)
    adata.uns['latent_dynamics']['conditional_probabilities'][
        'chain_given_nodes_selected'
    ] = observed_state_probs_mean.sum(0) / observed_state_probs_mean.sum(0).sum(0)
    adata.uns['latent_dynamics']['conditional_probabilities'][
        'chain_given_state_selected'
    ] = (
        observed_state_probs_mean.sum(-1).T / observed_state_probs_mean.sum(-1).sum(1)
    ).T
    adata.uns['latent_dynamics']['conditional_probabilities'][
        'chain_state_given_nodes_selected'
    ] = observed_state_probs_mean / observed_state_probs_mean.sum(0, keepdims=True).sum(
        1, keepdims=True
    )
    adata.uns['latent_dynamics']['conditional_probabilities'][
        'node_given_chain_state_selected'
    ] = observed_state_probs_mean / observed_state_probs_mean.sum(-1, keepdims=True)
    adata.uns['latent_dynamics']['conditional_probabilities'][
        'node_given_state_selected'
    ] = observed_state_probs_mean.sum(1) / observed_state_probs_mean.sum(1).sum(
        -1, keepdims=True
    )
...
def latent_state_selection(adata, states=None, criteria='argmax_joint', min_ratio=0.01):
    ...
    try:
        assert adata.uns['latent_dynamics']['model_outputs']
    except:
        raise ValueError('Model outputs are missing. Run infer_latent_dynamics() first')
    ...
    if criteria is None:
        ...
        selected_states = np.unique(
            adata.uns['latent_dynamics']['model_outputs']['viterbi_path'].flatten()
        )
    ...
    adata.uns['latent_dynamics']['posthoc_computations']['selected_states'] = (
        selected_states
    )
    ...
    if criteria is not None and min_ratio is None:
        if (
            ...
            logging.warning(
                'Number of kinetic states equals num_states. Consider initialising with more latent states'
            )
...
def infer_kinetic_clusters(
    data, states=None, criteria=None, min_ratio=0.01, copy=False
):
    ...
    try:
        ...
        raise ValueError(
            'Latent states could not be recovered. Run infer_latent_dynamics() first'
        )
    ...
    kinetic_states_probs = adata.uns['latent_dynamics']['conditional_probabilities'][
        'state_given_nodes_selected'
    ]
    ...
    lineage_probs = adata.uns['latent_dynamics']['conditional_probabilities'][
        'chain_given_nodes_selected'
    ]
    adata.obs['lineage_assignment'] = lineage_probs.argmax(0).flatten()
    adata.obs['lineage_assignment'] = adata.obs['lineage_assignment'].astype('category')

    adata.obs['kinetic_clustering'] = (
        adata.obs.kinetic_states.astype(str)
        + '_'
    ...
    adata.obs['transitional_entropy'] = entropy(
        adata.uns['latent_dynamics']['conditional_probabilities'][
            'state_given_nodes_selected'
        ]
    )
    ...
    adata.obs['differentiation_entropy'] = entropy(
        adata.uns['latent_dynamics']['conditional_probabilities'][
            'chain_given_nodes_selected'
        ]
    )
...
def infer_lineage_probabilities(data, use_selected=True, copy=False):
    ...
    if use_selected:
        try:
            ...
            raise ValueError(
                'Selected state conditional not found. Run compute_conditional(use_selected=True) first.'
            )
    ...
    adata.uns['latent_dynamics']['posthoc_computations']['lineage_probs'] = (
        lineage_probs
    )
...
def infer_latent_paths(data, criteria='argmax_joint', copy=False):
    ...
    selected_states_dict = dict(
        zip(np.arange(selected_states.shape[0]), selected_states)
    )
    ...
    if criteria is None or criteria == 'viterbi':
        ...
        lineage_paths = adata.uns['latent_dynamics']['model_outputs']['log_smoothing'][
            :, selected_states
        ].argmax(1)
    ...
    for lineage in range(
        adata.uns['latent_dynamics']['latent_dynamics_params']['num_chains']
    ):
        condensed_lineage_paths[lineage] = list(
            dict.fromkeys(lineage_paths[:, lineage])
        )

    adata.uns['latent_dynamics']['posthoc_computations']['latent_paths'] = lineage_paths
    adata.uns['latent_dynamics']['posthoc_computations']['condensed_latent_paths'] = (
        condensed_lineage_paths
    )
