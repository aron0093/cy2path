...
from .analytical import calculate_absorbing_probabilities, calculate_fundamental
...
def calculate_fate_probabilities(
    ...
    # Transition states
    transition_state_indices = list(
        set(np.arange(adata.shape[0])).difference(terminal_state_indices)
    )

    N = calculate_fundamental(T, transition_state_indices)
    B = calculate_absorbing_probabilities(
        N, T, transition_state_indices, terminal_state_indices
    )
...
def infer_fate_distribution(
    ...
    (
        ...
        adata, matrix_key=matrix_key, terminal_state_indices=terminal_state_indices
    )
    ...
    fate_probabilities[
        terminal_state_indices, np.arange(len(terminal_state_indices))
    ] = 1
