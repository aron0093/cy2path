...
class FHMM(torch.nn.Module):
    ...
    def __init__(
        ...
        self.unnormalized_state_init = torch.nn.Parameter(
            torch.randn(self.num_states, self.num_chains)
        )

        # Intialise the weights of each node towards each chain
        self.unnormalized_chain_weights = torch.nn.Parameter(
            torch.randn(  # self.num_iters,
                self.num_chains,
            )
        )
        ...
        self.unnormalized_transition_matrix = torch.nn.Parameter(
            torch.randn(self.num_chains, self.num_states, self.num_states)
        )
    ...
    def forward_model(self):
        ...
        self.log_transition_matrix = (
            self.log_transition_matrix_
            + self.log_chain_weights.unsqueeze(-1).unsqueeze(-1)
        ).logsumexp(0)

        # MSM iteration wise probability calculation
        log_hidden_state_probs = torch.zeros(
            self.num_iters, self.num_states, self.num_chains
        )
        log_observed_state_probs_ = torch.zeros(
            self.num_iters, self.num_states, self.num_chains, self.num_nodes
        )
        ...
        log_observed_state_probs_[0] = (
            self.log_emission_matrix + log_hidden_state_probs[0].unsqueeze(-1)
        )  #

        for t in range(1, self.num_iters):
            log_hidden_state_probs[t] = (
                (
                    log_hidden_state_probs[t - 1].transpose(1, 0).unsqueeze(-1)
                    + self.log_transition_matrix_
                )
                .logsumexp(1)
                .transpose(1, 0)
            ...
            log_observed_state_probs_[t] = (
                self.log_emission_matrix + log_hidden_state_probs[t].unsqueeze(-1)
            )
        ...
        # P(l)
        log_observed_state_probs_ = (
            log_observed_state_probs_
            + self.log_chain_weights.repeat(self.num_iters, 1)
            .unsqueeze(1)
            .unsqueeze(-1)
        )
        ...
        return (
            log_observed_state_probs,
            log_observed_state_probs_,
            log_hidden_state_probs,
        )
    ...
    def filtering(self, D):
        ...
        # Initialise at iteration 0
        log_alpha[0] = (
            D.log()[0].unsqueeze(0).unsqueeze(0)
            + self.log_emission_matrix
            + self.log_state_init.unsqueeze(-1)
        ).logsumexp(-1)
        ...
        for t in range(1, self.num_iters):
            log_alpha[t] = (
                D.log()[t].unsqueeze(0).unsqueeze(0)
                + self.log_emission_matrix
                + (
    ...
    def smoothing(self, D):
        ...
        # Initialise at iteration 0
        log_beta[-1] = (
            self.log_transition_matrix_.permute((1, 2, 0))
            + (
                D.log()[-1].unsqueeze(0).unsqueeze(0)
                + self.log_emission_matrix
                + init.unsqueeze(-1).unsqueeze(-1)
            )
            .logsumexp(-1)
            .unsqueeze(0)
        ).logsumexp(1)
        ...
        for t in range(self.num_iters - 1, 0, -1):
            ...
            log_gamma[t - 1] = (
                log_beta[t - 1] - log_probs.sum(0).unsqueeze(0) + log_alpha[t - 1]
            )
    ...
    def viterbi(self, D):
        ...
        for t in range(1, self.num_iters):
            max_val, argmax_val = torch.max(
                log_delta[t - 1].transpose(1, 0).unsqueeze(-1)
                + self.log_transition_matrix_,
                dim=1,
            )
            ...
            log_delta[t] = (
                D.log()[t].unsqueeze(0).unsqueeze(0)
                + self.log_emission_matrix
                + max_val.transpose(1, 0).unsqueeze(0).unsqueeze(-1)
            ).logsumexp(-1)
    ...
    def train(
        ...
        num_epochs=500,
        sparsity_weight=1.0,
        exclusivity_weight=1e-1,
        orthogonality_weight=1e-1,
        TPM_weight=0.0,
        optimizer=None,
        criterion=None,
        swa_scheduler=None,
