...
class LSSM(torch.nn.Module):
    ...
    and state level chain weights trained on
    ...
    def __init__(
        ...
        self.unnormalized_transition_matrix = torch.nn.Parameter(
            torch.randn(self.num_states, self.num_states)
        )
    ...
    def forward_model(self):
        ...
        log_observed_state_probs_[0] = (
            self.log_emission_matrix + self.log_chain_weights.unsqueeze(-1)
        ) + log_hidden_state_probs[0].unsqueeze(-1).unsqueeze(-1)

        for t in range(1, self.num_iters):
            log_hidden_state_probs[t] = (
                log_hidden_state_probs[t - 1].unsqueeze(-1)
                + self.log_transition_matrix.unsqueeze(0)
            ).logsumexp(1)
    ...
    def filtering(self, D):
        ...
        log_alpha[0] = (
            ...
            + self.log_state_init.unsqueeze(-1).unsqueeze(-1)
        ).logsumexp(-1)
    ...
    def smoothing(self, D):
        ...
        # Initialise at iteration 0
        log_beta[-1] = (
            self.log_transition_matrix.unsqueeze(-1)
            + (
