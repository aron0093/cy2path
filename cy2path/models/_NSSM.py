...
class NSSM(torch.nn.Module):
    ...
    def __init__(
        ...
        self.unnormalized_emission_matrix = torch.nn.Parameter(
            torch.randn(self.num_states, self.num_nodes)
        )
    ...
    def forward_model(self):
        ...
        # Joint chain, node
        self.log_weights = (
            self.log_emission_matrix.unsqueeze(-1) + self.log_chain_weights
        )
        ...
        log_observed_state_probs_ = torch.zeros(
            self.num_iters, self.num_states, self.num_nodes, self.num_chains
        )
        ...
        log_observed_state_probs_[0] = self.log_weights + log_hidden_state_probs[
            0
        ...
        for t in range(1, self.num_iters):
            ...
            log_observed_state_probs_[t] = self.log_weights + log_hidden_state_probs[
                t
            ].unsqueeze(-1).unsqueeze(-1)
    ...
    def filtering(self, D):
        ...
        # Initialise at iteration 0
        log_alpha[0] = (
            D.log()[0].unsqueeze(0).unsqueeze(-1)
            + self.log_weights
        ...
        for t in range(1, self.num_iters):
            log_alpha[t] = (
                D.log()[t].unsqueeze(0).unsqueeze(-1)
                + self.log_weights
                + (
    ...
    def smoothing(self, D):
        ...
        log_beta[-1] = (
            ...
            + (
                D.log()[-1].unsqueeze(0).unsqueeze(-1)
                + self.log_weights
    ...
    def viterbi(self, D):
        ...
        for t in range(1, self.num_iters):
            ...
            log_delta[t] = (
                D.log()[t].unsqueeze(0).unsqueeze(-1)
                + self.log_weights
                + max_val.transpose(1, 0).unsqueeze(1)
            ).logsumexp(1)
