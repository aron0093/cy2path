...
from ..utils import JSDLoss, log_domain_mean
...
def train(
    ...
    try:
        assert self.elapsed_epochs
    except:
        self.elapsed_epochs = 0

    try:
        assert self.loss_values
    except:
        self.loss_values = []

    try:
        assert self.divergence_values
    except:
        self.divergence_values = []

    try:
        assert self.likelihood_values
    except:
        self.likelihood_values = []

    try:
        assert self.sparsity_values
    except:
        self.sparsity_values = []

    try:
        assert self.orthogonality_values
    except:
        self.orthogonality_values = []

    try:
        assert self.exclusivity_values
    except:
        self.exclusivity_values = []

    try:
        assert self.regularisation_values
    except:
        self.regularisation_values = []
    ...
    for epoch in tqdm(
        ...
        # Model output
        prediction, log_observed_state_probs_, log_hidden_state_probs = (
            self.forward_model()
        )
        ...
        log_observed_state_probs_mean = log_domain_mean(
            log_observed_state_probs_, use_gpu=self.is_cuda
        )

        log_nodes_given_state = log_observed_state_probs_mean.logsumexp(
            1
        ...
        if self.num_chains > 1:
            log_nodes_given_chain = log_observed_state_probs_mean.logsumexp(
                0
            ) - log_observed_state_probs_mean.logsumexp(0).logsumexp(-1, keepdims=True)

            log_chains_given_node = log_observed_state_probs_mean.logsumexp(
                0
            ...
            right_square_left = torch.matmul(
                right_square, log_chains_given_node.transpose(1, 0).exp()
            )

            exclusivity = self.criterion(
                torch.diag(right_square_left.log()), chain_identity
            )
        ...
        regularisation = self.criterion(
            log_nodes_given_state,
            torch.matmul(torch.exp(log_nodes_given_state.detach()), TPM),
        )
