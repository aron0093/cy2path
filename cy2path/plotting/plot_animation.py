...
def make_frame(
    ...
    if rescale_color:
        scatter(
            adata,
            color='current_state_probability',
            title='Iteration: {}'.format(i),
            dpi=dpi,
            figsize=figsize,
            show=False,
            save=os.path.join(save, '{}.png'.format(i)),
            basis=basis,
            color_map=color_map,
            rescale_color=rescale_color,
            vmax=rescale_color[1],
            vmin=rescale_color[0],
        )
    else:
        scatter(
            adata,
...
def plot_animation(
    ...
    figsize=(7, 5),
    color_map='YlGnBu',
    gradient_frames=False,
    gradient_elongations=False,
    framerate=0.2,
    rescale_color=[0, 1],
    save='./temp/',
    n_jobs=-1,
    ...
    Parallel(n_jobs=n_jobs)(
        ...
        for i in tqdm(
            range(data_history.shape[0]),
            desc='Creating frame of each iteration',
            unit=' frames',
        )
    )

    filenames = [
        os.path.join(save, '{}.png'.format(i)) for i in range(data_history.shape[0])
    ]
    # Add gradient to selected frames
    if gradient_frames and gradient_elongations:
        filenames = make_gradient(
            frames=filenames,
            target_frames=gradient_frames,
            frame_elongations=gradient_elongations,
        )
