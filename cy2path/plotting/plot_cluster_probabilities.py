...
def plot_probability_by_clusters(
    adata,
    cluster_key: str,
    key='state_history',
    plot='heatmap',
    ...
    if plot == 'heatmap':
        ...
        state_history_clusters = state_history_clusters.reset_index().melt(
            id_vars=cluster_key, var_name='iteration'
        )
        hue_categories = (
            state_history_clusters[cluster_key].astype('category').cat.categories
        )
        palette = dict(zip(hue_categories, adata.uns['{}_colors'.format(cluster_key)]))
        sns.lineplot(
            x='iteration',
            y='value',
            data=state_history_clusters,
            hue=cluster_key,
            palette=palette,
            ax=ax,
        )
