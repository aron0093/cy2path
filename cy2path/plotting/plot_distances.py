...
def plot_distances(
    ...
    if mode == 'sampling':
        ...
        if (
            adata.uns[upper_key]['sampling_params']['convergence_criterion']
            in adata.obs.columns
        ):
            stationary_state_key = 'stationary_state_probability_by_cluster'

    try:
        data_history = adata.uns[upper_key][key]
    except:
        raise ValueError(
            '{} could not be recovered. Run corresponding function first'.format(
    ...
    for i in range(1, data_history.shape[0]):
        if metric == 'cosine':
            ...
            distance_to_stationary.append(
                cosine(data_history[i], adata.uns[upper_key][stationary_state_key])
            )
        elif metric == 'euclidean':
            distance_to_next.append(euclidean(data_history[i - 1], data_history[i]))
            distance_to_stationary.append(
                euclidean(data_history[i], adata.uns[upper_key][stationary_state_key])
            )

    distance_to_stationary_iterative = [
        abs(distance_to_stationary[i + 1] - distance_to_stationary[i])
        for i in range(len(distance_to_stationary) - 1)
    ]
    ...
    axs[2].set_title(
        '{} distance to stationary state: [i] - [i-1]'.format(metric.capitalize())
    )
    axs[2].plot(distance_to_stationary_iterative)

    for ax in axs:
        ax.axvline(
            x=adata.uns[upper_key]['sampling_params']['convergence'],
            linestyle='--',
            color='darkgrey',
        )
        if log:
