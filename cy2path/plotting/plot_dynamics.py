...
def plot_loss(model, figsize=(15, 7)):
    ...
    sns.lineplot(
        np.log(np.array(model.loss_values) + model.loss_values[-1] / 10), ax=axs.flat[0]
    )
...
def plot_ratio(adata, probability=False, ax=None):
    if probability:
        ...
        state_ratio = np.empty(
            adata.uns['latent_dynamics']['latent_dynamics_params']['num_states']
        )
    ...
    sns.lineplot(
        x=state_ratio_args.astype(str),
        y=state_ratio[state_ratio_args],
        color='black',
        ax=ax,
    )
    sns.scatterplot(
    ...
    if adata.uns['latent_dynamics']['latent_dynamics_params']['min_ratio']:
        ax.hlines(
            adata.uns['latent_dynamics']['latent_dynamics_params']['min_ratio'],
            0,
...
def plot_latent_transition_matrix(adata, states=None, ax=None):
    ...
    try:
        assert adata.uns['latent_dynamics']['model_params']
    except:
        raise ValueError(
            'Latent state TPM not found. Run infer_latent_dynamics() first'
        )

    latent_transition_matrix = adata.uns['latent_dynamics']['model_params'][
...
def plot_latent_transitions(adata, ax=None):
    ...
    try:
        assert (
            'latent_paths'
            in adata.uns['latent_dynamics']['posthoc_computations'].keys()
        )
    except:
        raise ValueError('Latent paths not found. Run infer_latent_paths() first')

    sns.lineplot(
        adata.uns['latent_dynamics']['posthoc_computations']['latent_paths'].astype(
            int
        ),
        ax=ax,
    )
...
def plot_state_assignment(adata, states=None, prob_key='chain_given_state', ax=None):
    ...
    try:
        assert (
            prob_key in adata.uns['latent_dynamics']['conditional_probabilities'].keys()
        )
    except:
        raise ValueError('Conditionals not found. Run infer_latent_dynamics() first')
...
def plot_latent_paths(
    adata, color='whitesmoke', mode='clusters', basis='umap', ax=None
):
    # Check if latent path exists
    try:
        assert (
            'condensed_latent_paths'
            in adata.uns['latent_dynamics']['posthoc_computations'].keys()
        )
    except:
        raise ValueError('Latent paths not found. Run infer_latent_paths() first')

    lineage_paths = adata.uns['latent_dynamics']['posthoc_computations'][
        'condensed_latent_paths'
    ]
    ...
    for lineage, path in lineage_paths.items():
        node_coordinates = pd.DataFrame(
            index=path, columns=np.arange(adata.obsm['X_{}'.format(basis)].shape[1])
        )
        for state in path:
            if mode == 'clusters':
                # Chain specific weight of nodes to calculate coords
                state_probs = adata.uns['latent_dynamics']['conditional_probabilities'][
                    'node_given_chain_state_selected'
                ][state, lineage]
            elif mode == 'states':
                # Overall coordinate of state
                state_probs = adata.uns['latent_dynamics']['conditional_probabilities'][
                    'node_given_state_selected'
                ][state]

            node_coordinates.loc[state] = np.sum(
                np.multiply(
                    state_probs.reshape(-1, 1), adata.obsm['X_{}'.format(basis)]
                ),
                axis=0,
            )
            sns.scatterplot(
                x=[node_coordinates.loc[state][0]],
        ...
        for step in range(len(path) - 1):
            ax.arrow(
                node_coordinates.loc[path[step], 0],
                node_coordinates.loc[path[step], 1],
                node_coordinates.loc[path[step + 1], 0]
                - node_coordinates.loc[path[step], 0],
                node_coordinates.loc[path[step + 1], 1]
                - node_coordinates.loc[path[step], 1],
                color=color_range[lineage],
...
def plot_annotation_boxplots(
    ...
    flierprops = dict(
        marker='o', markerfacecolor='None', markersize=0.5, markeredgecolor='black'
    )

    if order is None:
        order = (
            adata.obs.groupby(categorical)[continous]
            .mean()
            .fillna(0)
            .sort_values()
            .index
...
def plot_static_kinetic_overlap(
    adata, cluster_key: str, prob_key='state_given_nodes', states=None, ax=None
):
    # Check if probs exist
    try:
        assert (
    ...
    probs = pd.DataFrame(
        adata.uns['latent_dynamics']['conditional_probabilities'][prob_key].T,
    ...
    sns.heatmap(probs.T.astype(float), annot=True, cmap='YlGnBu', vmax=1, vmin=0, ax=ax)
    ax.set_yticklabels(states)
    ax.set_xlabel('')
...
def plot_dynamics_summary(
    ...
    if cluster_key not in adata.obs.columns:
        raise KeyError(
            'Key {} for cluster annotations not found in adata.obs.'.format(cluster_key)
        )
    ...
    if use_selected:
        try:
            ...
            raise ValueError('No selected states. Run latent_state_selection() first')
        selected_states = adata.uns['latent_dynamics']['posthoc_computations'][
            'selected_states'
        ]
    ...
    plot_state_assignment(
        adata,
        prob_key=plot_state_assignment_key,
    ...
    order = (
        adata.obs.groupby('kinetic_states')['pseudotime']
        .mean()
        .fillna(0)
        .sort_values()
        .index
    ...
    # Annotation vs pst
    order = (
        adata.obs.groupby(cluster_key)['pseudotime']
        .mean()
        .fillna(0)
        .sort_values()
        .index
    ...
    plot_static_kinetic_overlap(
        ...
        cluster_key,
        states=selected_states,
        prob_key=plot_static_kinetic_overlap_key,
        ax=axs.flat[8],
    )
    ...
    t_entropy_df = (
        adata.obs.groupby(['kinetic_states', cluster_key]).count().reset_index()
    )
    t_entropy_df = (
        t_entropy_df.pivot(
            index='kinetic_states', columns=cluster_key, values='transitional_entropy'
        )
        .astype(float)
        .round(2)
    )
    t_entropy_df = t_entropy_df.divide(
        adata.obs.groupby(cluster_key).count().transitional_entropy, axis=1
    )

    sns.heatmap(t_entropy_df, cmap='YlGnBu', annot=True, ax=axs.flat[11])
    axs.flat[11].set_title(' Trans. Entropy of annot. per states')

    # Differentiation Entropy on UMAP
    scatter(
        adata,
        color='differentiation_entropy',
        legend_loc='on data',
        basis=basis,
        alpha=0.8,
        show=False,
    ...
    plot_annotation_boxplots(
        adata,
        categorical=cluster_key,
        continous='differentiation_entropy',
        order=order,
        color='salmon',
        ax=axs.flat[13],
    )
    ...
    d_entropy_df = (
        d_entropy_df.pivot(
            index='kinetic_states',
            columns=cluster_key,
            values='differentiation_entropy',
        )
        .astype(float)
        .round(2)
    )
    d_entropy_df = d_entropy_df.divide(
        adata.obs.groupby(cluster_key).count().differentiation_entropy, axis=1
    )

    sns.heatmap(d_entropy_df, cmap='YlGnBu', annot=True, ax=axs.flat[14])
...
def plot_latent_summary(
    adata, use_selected=True, ncols=4, basis='umap', figsize=(5, 3.5)
):
    # Check if probs exist
    try:
        assert (
            'state_given_nodes'
            in adata.uns['latent_dynamics']['conditional_probabilities'].keys()
        )
    except:
        raise ValueError('Conditionals not found. Run infer_latent_dynamics() first')

    probs = adata.uns['latent_dynamics']['conditional_probabilities'][
        'state_given_nodes'
    ]
...
def plot_latent_state_history(adata, states=None, chain=None, ax=None):
    ...
    try:
        assert (
            'joint_probabilities'
            in adata.uns['latent_dynamics']['model_outputs'].keys()
        )
    except:
        raise ValueError('Probabilities not found. Run infer_latent_dynamics() first')
...
def plot_lineages_summary(
    ...
    for i in range(num_chains):
        adata.uns['state_probability_sampling']['state_history_chain_{}'.format(i)] = (
            adata.uns['latent_dynamics']['model_outputs']['joint_probabilities'].sum(1)[
                :, i, :
            ]
        )
        ...
        scatter(
            adata,
            basis=basis,
            color=adata.uns['latent_dynamics']['posthoc_computations']['lineage_probs'][
                i
            ],
        ...
        plot_latent_state_history(
            adata, states=selected_states, chain=i, ax=axs.flat[i * 4 + 2]
        )
...
# Plot simulation
def plot_simulation(
    adata, prob_key='state_history', color='whitesmoke', basis='umap', ax=None
):
    # Check if state history exists
    try:
        assert prob_key in adata.uns['state_probability_sampling'].keys()
    except:
        raise ValueError('{} not found. Run sampling/dynamics first'.format(prob_key))
    ...
    for step in node_coordinates.index.values:
        node_coordinates.loc[step] = np.sum(
            np.multiply(probs[step].reshape(-1, 1), adata.obsm['X_{}'.format(basis)]),
            axis=0,
        )
        sns.scatterplot(
            x=[node_coordinates.loc[step][0]],
