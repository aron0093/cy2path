...
def plot_fate(
    ...
    try:
        fate_probabilities = adata.uns[upper_key]['fate_probabilities_macro']
    except:
        raise ValueError('Fate probabilities were not found.')
    ...
    fig, axs = plt.subplots(
        ncols=ncols, nrows=nrows, figsize=(6 * ncols, 5 * nrows), dpi=dpi
    )
    ...
    for i, idx in enumerate(fate_probabilities.index.values):
        scatter(
            adata,
            basis=basis,
            color=fate_probabilities.loc[idx].astype(float),
            title='Fate probabilities: {}'.format(idx),
            color_map=color_map,
            ax=axs[i],
            show=False,
        )
