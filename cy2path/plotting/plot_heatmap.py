...
def plot_persistence_heatmap(
    ...
    if row_color_key is not None:
        ...
        try:
            ...
            logging.warning(
                'Key {} for row colors not found in adata.obs.'.format(row_color_key)
            )
        except AttributeError:
            logging.info(
                'Converting adata.obs["{}"] to categorical.'.format(row_color_key)
            )
        ...
        if row_color_categories is not None:
            try:
                row_color_map = dict(
                    zip(
                        row_color_categories,
                        adata.uns['{}_colors'.format(row_color_key)],
                    )
                )
            except:
                logging.info(
                    'Colors not found, creating colors for {} and storing to adata.uns.'.format(
                        row_color_key
                    )
                )
                ...
                if n_cat <= 10:
                    row_color_map = dict(
                        zip(row_color_categories, [rgb2hex(c) for c in cm.tab10.colors])
                    )
                else:
                    row_color_map = dict(
                        zip(row_color_categories, [rgb2hex(c) for c in cm.tab20.colors])
                    )
                adata.uns['{}_colors'.format(row_color_key)] = np.asarray(
                    row_color_map.values()
                )
    ...
    if row_colors is not None:
        for label in row_color_map.keys():
            g.ax_col_dendrogram.bar(
                0, 0, color=row_color_map[label], label=label, linewidth=0
            )
