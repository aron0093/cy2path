...
def infer_pseudotime(data, mode="weighted_mean", copy=False):
    ...
    if mode == "weighted_mean":
        _ = np.repeat(
            [np.arange(0, state_history.shape[0])], state_history.shape[1], 0
        ).T
        _ = _ * state_history
...
def add_expected_steps(data, copy=False):
    ...
    try:
        N = adata.uns["cell_fate"]["fundamental_matrix"]
    except:
        raise ValueError(
            "Fundamental matrix could not be recovered. Run infer_fate_distribution() first"
        )
