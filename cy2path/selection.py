...
def run_search(
    ...
    **kwargs,
    ...
    fit_values = pd.DataFrame(
        product(*(param_list[key] for key in param_list)),
        columns=list(param_list.keys()),
    )
    fit_values[
        [
            'avg_corrcoef',
    ...
    for idx in fit_values.index.values:
        ...
        if 'exclusivity_weight' in fit_values.columns:
            train_params['exclusivity_weight'] = fit_values.loc[
                idx, 'exclusivity_weight'
            ]
        if 'orthogonality_weight' in fit_values.columns:
            train_params['orthogonality_weight'] = fit_values.loc[
        ...
        state_ratio = np.empty(
            adata_.uns['latent_dynamics']['latent_dynamics_params']['num_states']
        )
        states, counts = np.unique(
            adata_.uns['latent_dynamics']['conditional_probabilities'][
                'state_given_nodes'
            ].argmax(0),
            return_counts=True,
        )
        ...
        infer_kinetic_clusters(
            adata_, states=None, min_ratio=0.0, criteria='argmax_joint'
        )
