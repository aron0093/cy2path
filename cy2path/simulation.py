...
from .sampling import check_convergence_criteria, iterate_state_probability
...
def extract_nonzero_entries(adata, matrix_key='T_forward'):
    ...
    trans_matrix_probabilites = np.split(
        adata.obsp[matrix_key].data, adata.obsp[matrix_key].indptr
    )[1:-1]
    ...
    trans_matrix_indice_nonzeros = np.zeros(
        (
            len(trans_matrix_probabilites),
            max(np.count_nonzero(adata.obsp[matrix_key].toarray(), axis=1)),
        ),
        dtype=np.int32,
    )
    trans_matrix_probabilites_nonzeros = np.zeros(
        (
            len(trans_matrix_probabilites),
            max(np.count_nonzero(adata.obsp[matrix_key].toarray(), axis=1)),
        ),
    ...
    trans_matrix_probabilites_nonzeros = np.round(
        trans_matrix_probabilites_nonzeros, decimals=3
    )
...
def iterate_markov_chain(
    ...
    if nonzero_entries is None:
        trans_matrix_indice_nonzeros, trans_matrix_probabilites_nonzeros = (
            extract_nonzero_entries(adata, matrix_key=matrix_key)
        )
    else:
        trans_matrix_indice_nonzeros, trans_matrix_probabilites_nonzeros = (
            nonzero_entries
    ...
    for k in range(1, max_iter):
        ...
        state_indices_[k] = trans_matrix_indice_nonzeros[
            state_indices_[k - 1],
            np.where(
                random_step[k - 1]
                <= trans_matrix_probabilites_nonzeros[state_indices_[k - 1], :]
            )[0][0],
        ]
        state_transition_probabilities_[k - 1] = adata.obsp[matrix_key][
            state_indices_[k - 1], state_indices_[k]
        ]
...
def sample_markov_chains(
    ...
    stationary_state_probability = estimate_stationary_state(
        adata, matrix_key=matrix_key
    )

    # Create empty arrays for the simulation and sample random numbers for all samples and max steps
    state_transition_probabilities = np.empty(
        (num_chains, max_iter - 1), dtype=np.float32
    )
    ...
    simulations = Parallel(n_jobs=n_jobs)(
        delayed(iterate_markov_chain)(
            adata,
            matrix_key=matrix_key,
            max_iter=max_iter,
            init_state=np.random.choice(
                adata.shape[0], 1, replace=repeat_root, p=init_state_probability
            ),
            nonzero_entries=(
                trans_matrix_indice_nonzeros,
                trans_matrix_probabilites_nonzeros,
            ),
        )
        for i in tqdm(
    ...
    if convergence == 'auto':
        convergence_check = iterate_state_probability(
            adata,
            matrix_key=matrix_key,
            init=init_state_probability,
            stationary=stationary_state_probability,
            max_iter=max_iter,
            tol=tol,
        )[-1]
