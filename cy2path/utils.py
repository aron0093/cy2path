...
def check_TPM(
    ...
    if recalc_matrix:
        adata.obsp[matrix_key] = get_transition_matrix(
            adata, self_transitions=self_transitions
        )
...
def log_domain_hardmax(tensor_, dim=0):
    ...
    log_hardmax = (
        torch.nn.functional.one_hot(argmax_, num_classes=num_cats)
        .log()
        .reshape(tensor_.shape)
    )
    return log_hardmax
...
class JSDLoss(torch.nn.Module):
    ...
    def forward(self, tensor):
        ...
        return weight * sum(
            [self.kl(centroid, tensor[i]) for i in range(tensor.shape[0])]
        )
        ...
        for i in range(contingency_table.shape[0]):
            for j in range(contingency_table.shape[1]):
                if contingency_table[i, j] > 0:
                    mutual_info += contingency_table[i, j] * torch.log2(
                        contingency_table[i, j] / (marginal_x[i] * marginal_y[j])
                    )
